...
for(int i=1;i<Info.numPages;i++){
	...
	public static void getPageTableTexture(GL gl, int texture_index){
    	...
    	//System.out.println("pixels.capacity() "+pageTableTexture.capacity());
    	...
    	for(int i=0; i<pageTableTexture.capacity(); i+=3) {
    		...
    		//normalizzo per avere valori da 0 a 1, poi moltiplico per il range dei canali: 0 a 255
    		int r2=(int)(((float)r/Info.w_pageCache)*255);
    		int g2=(int)(((float)g/Info.h_pageCache)*255);
    		//int b2=(int)(((float)b)*255);
    		int b2=(int)(((float)b/Info.numLevels)*255);
	...
	public static boolean isAlreadyInCache(int absIndex,int level){
		//boolean isIn=(pageTable[absIndex].isInCache&&pageTable[absIndex].level==level);
		boolean isIn=(pageTable[absIndex].isInCache);
		return isIn;
	}
	
	public static boolean isAlreadyLoaded(int absIndex,int level){
		//boolean isIn=(pageTable[absIndex].isInCache&&pageTable[absIndex].level==level);
		boolean isIn=(pageTable[absIndex].isLoaded);
		return isIn;
	}
	...
	public static void isloaded(int absIndex,int level){
		//System.out.println("absolute index= "+ absIndex);
		pageTable[absIndex].level=level;
		pageTable[absIndex].isLoaded=true;
	}
